...
def dashboard(request):
    ...
    try:
        ...
        if request.user.is_authenticated:
            ...
            if not request.user.is_superuser:
                ...
                if not emp_obj:
                    ...
                    sub_connects = Connect.objects.filter(
                        employee=profile_obj).order_by('-id')
        ...
        messages.warning(
            request, 'Somthing Went Wrong! Please Try after some time')
    return redirect('/')
            ...
            if not request.user.is_superuser:
                try:
                    events = Events.objects.exclude(
                        is_completed=True).order_by('-id')[:10]
                    ...
                    messages.warning(
                        request, 'Profile Not found Please contact Admin')
    ...
    try:
        if not request.user.is_superuser:
            ...
            if request.method == "POST":
                if number_of_documents.count() == 10:
                    messages.warning(
                        request, "You can't Add More than 10 Documents!")
                ...
                obj = Document.objects.create(
                    employee=employee, name=document_name, document=document)
                if obj:
...
def adminDocument(request, empid):
    ...
    try:
        if not request.user.is_superuser:
            profile_obj = Employee.objects.get(user=request.user)
            context['profile'] = profile_obj
            res = checkPermission(profile_obj, 'Employee')
            print('response ', res)
            if res == 'no':
                messages.warning(request, 'Sorry, You are not authorised!')
                return redirect('/')
...
def createDepartment(request):
    ...
    if not request.user.is_superuser:
        profile_obj = Employee.objects.get(user=request.user)
        context['profile'] = profile_obj
        res = checkPermission(profile_obj, 'Department')
        print('response ', res)
        if res != 'edit':
            messages.warning(request, 'Sorry, You are not authorised!')
            return redirect('/')
...
def viewDepartment(request):
    ...
    try:
        ...
        if request.method == 'POST':
            if res == 'view' or res == 'no':
                messages.warning(request, "Sorry, You don't have edit permission!")
                return redirect('ems:view-department')
...
def deleteDepartment(request, pk):
    try:
        if not request.user.is_superuser:
            ...
            print('response ', res)
            if res == 'view' or res == 'no':
                messages.warning(request,"Sorry, You don't have Delete permission!")
                return redirect('ems:view-department')
...
def createRole(request):
    ...
    if not request.user.is_superuser:
        profile_obj = Employee.objects.get(user=request.user)
        context['profile'] = profile_obj
        res = checkPermission(profile_obj, 'Role')
        print('response ', res)
        if res != 'edit':
            messages.warning(request, 'Sorry, You are not authorised!')
            return redirect('/')
        ...
        if request.method == 'POST':
            ...
            if full_name and father_name and dob and email and phone_number and address and street and locality and city and state and pincode and country and designation and department and role and joining_date and status:
                ...
                if employeeid == '':
                    ...
                    check_employeeid = Employee.objects.filter(
                        employeeid=employeeid).exists()

                    if check_employeeid:
                        if employee_obj.employeeid != int(employeeid):
                            messages.warning(
                                request, 'This Employee Id Already Exits! Please Try Another')
                            return redirect('ems:employee-detail', employee_obj.empid)
                    else:
...
def attendance(request):
    ...
    try:
        if not request.user.is_superuser:
            profile_obj = Employee.objects.get(user=request.user)
            context['profile'] = profile_obj 
            res = checkPermission(profile_obj, 'Clockify')
            print('response ', res)
            if res == 'no':
                messages.warning(request, 'Sorry, You are not authorised!')
                return redirect('/')
        ...
        if request.method == 'POST':
            if res == 'view' or res == 'no':
                messages.warning(request, "Sorry, You don't have Add Clokify permission!")
...
def attendanceDateFilter(request):
    context = {}
    try:
        filter_date = request.POST['filter-date']
        print('filter date ', filter_date)
        attendances = Attendance.objects.filter(
            date=filter_date).order_by('employee')
...
def editAttendance(request):
    ...
    try:
        ...
        if request.method == 'POST':
            if res == 'view' or res == 'no':
                messages.warning(request, "Sorry, You don't have edit Clokify permission!")
                return redirect('ems:attendance')
...
def allAttendances(request):
    ...
    try:
        if not request.user.is_superuser:
            ...
            emp_obj = Employee.objects.get(user=request.user)
            attendances = Attendance.objects.filter(
                employee=emp_obj).order_by('-date')
...
def createLeave(request):
    ...
    try:
        if not request.user.is_superuser:
            ...
            try:
                allocated_leaves = AllocatedLeave.objects.get(
            ...
            messages.warning(request, 'You are not Authrised!')
            return redirect('/')
...
def dashboardLeaves(request):
    ...
    try:
        if not request.user.is_superuser:
            profile_obj = Employee.objects.get(user=request.user)
            context['profile'] = profile_obj
            res = checkPermission(profile_obj, 'Leaves')
            print('response ', res)
            if res == 'no':
                messages.warning(request, 'Sorry, You are not authorised!')
                return redirect('/')
    ...
    if request.user.is_superuser:
        try:
            ...
            if request.method == 'POST':
                ...
                employee_obj = Employee.objects.get(empid=empid)
                check = AllocatedLeave.objects.filter(
                    employee=employee_obj).exists()
                if check:
                    messages.warning(
                        request, 'This Employee has Already Allocated Leaves! Please try another.')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            obj = Events.objects.create(
                title=title, description=description, date=date, image=image)
            print(obj)
            if obj:
                ...
                messages.warning(
                    request, 'Somthing went Wrong! Please Try Again.')
    except Exception as e:
            ...
            if profile_obj.role.name != 'Manager':
                ...
                if request.method == 'POST':
                    ...
                    if obj:
                        ...
                        messages.warning(
                            request, 'Something Went Wrong! Please try after some time!')
            else:
...
def quriesManger(request):
    ...
    try:
        if not request.user.is_superuser:
            ...
            context['profile'] = profile_obj
            if profile_obj.role.name == 'Manager':
                queries = DepartmentQuery.objects.filter(
                    department=profile_obj.department).order_by('-id')
                context['queries'] = queries
                if request.method == 'POST':
                ...
                messages.warning(
                    request, "You don't have Permission to access!")
                return redirect('ems:ems')
        else:
        ...
        if not request.user.is_superuser:
            ...
            if request.method == 'POST':
                ...
                if obj:
                    print(obj)
                    messages.success(
                        request, 'Reimbursement Requested Successfully!')
...
def reimbursementTransportAll(request):
    ...
    try:
        if not request.user.is_superuser:
            profile_obj = Employee.objects.get(user=request.user)
            context['profile'] = profile_obj
            cabreimbursements = ReimbursementTransport.objects.filter(
                employee=profile_obj).order_by('-id')
        ...
        if not request.user.is_superuser:
            ...
            if request.method == 'POST':
                ...
                if x > 200:
                    ...
                    messages.warning(
                        request, 'Your Amount is Exceeding! Per Person Allowed only ₹200/. ')
                    bill = 'food'
                    return redirect('ems:reimbursement-bill', bill)
                else:
...
def reimbursementFoodAll(request):
    ...
    try:
        if not request.user.is_superuser:
            ...
            reimbursement_foods = ReimbursementFood.objects.filter(
                employee=profile_obj).order_by('-id')
...
def reimbursementEmployee(request, id):
    ...
    try:
        ...
        print(obj)
        reimbursements_employees = ReimbursementFoodEmployee.objects.filter(
            reimbursement_food=obj)
    ...
    if not request.user.is_superuser:
        try:
            ...
            if request.method == 'POST':
                ...
                allattendances = Attendance.objects.filter(
                    employee=profile_obj)
                yearAttendances = allattendances.filter(date__year=year)
                monthAttendances = yearAttendances.filter(date__month=month)
                a = allattendances.count()
